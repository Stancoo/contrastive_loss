...
As an input let the model take a sequence of frames of the fixed length, defined in `config.json` as `"seq_len":21`. The sequences are produced by replicating the same "coin" image `img.png` rotated by, say, two degrees from frame to frame:
