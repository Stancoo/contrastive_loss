...
At the same time, we also observe some side-effects. Contrastive treating of the defect group affects few more outlier groups in a similar way. These are the $3$-degree (blue) as well as the completely random rotations (turquoise). As we see in the explicit example, the three corresponding different inputs ($2.1$, $3$, random) obtain the same recovery. This indicates that the whole certain subarea of the latent space is transformed by the decoder almost into a single point of the "real" space.