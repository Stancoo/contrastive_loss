...
It is important to admit, that the contrastive loss is not a real loss in the sense of the AE paradigm, i.e., it does not care about the input recovery as the very first term ("traditional") in the total loss expression above does. Contrastive loss is just a penalty which only tries to make the defects as distinct as possible from the main-group elements.
