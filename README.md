...
To demonstrate the advantage of the contrastive loss account on a particular example, as a  <a href="model_convlstm_ae.py">model</a> we adapt a version of the so-called  <a href="https://github.com/HSoo-Kim/SpatioTemporal-AutoEncoder"> Spatio-Temporal AE</a> translated into Pytorch. The model is intended to binary classify the video fragments, i.e. as "normal/anomal". Training and testing procedures are configured in `config.json`:
